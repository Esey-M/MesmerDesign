...
3. **Add to `data/products.yaml`** (keyed by id): `title, url (Etsy listing URL — use the shop-subdomain host `https://mesmerdesing.etsy.com/listing/...`, NOT `www.etsy.com`, for the Share & Save fee discount [[etsy-share-save-urls]]; UTM added automatically), price ("21,77"), image (/img/products/<id>.jpg), themes[], recipients[], occasions[], product_types[]`.
4. **Category**: place post in an existing `content/blog/<category>/` section. Categories so far: gifts-for-him, gifts-for-her, christian-gifts, business-gifts, funny-gifts, personalized-gifts, teacher-appreciation-gifts, christmas-gifts, graduation-gifts, new-job-gifts, jigsaw-puzzle-gifts, wall-art. **If a new category fits better**, create `content/blog/<cat>/_index.md` (pillar: title, linkTitle, description, keywords, 2-para intro) AND add it to the `Gift Guides` mega-menu in `config/_default/menus.toml` (a `[[main]]` entry with `parent = "gift-guides"` and a `group` param: "By recipient" / "By occasion" / "By style").
